...
def showWeizhen():
    ...
    limit2 = page_size
    select_sql_all = "SELECT * FROM weizhen "
    contents_all = db.select_db(select_sql_all)
    count = len(contents_all)
    select_sql = "SELECT * FROM weizhen LIMIT {},{}".format(limit1,limit2)
    # 数据库返回得到的内容
    contents = db.select_db(select_sql)
...
def showInquireWeizhen():
    ...
    select_sql_all = "SELECT * FROM weizhen WHERE {} = {}".format(name,value)
    contents_all = db.select_db(select_sql_all)
    count = len(contents_all)
    select_sql = 'SELECT * FROM weizhen WHERE {} = {} LIMIT {},{}'.format(name,value,limit1,limit2)
    # 数据库返回得到的内容
    contents = db.select_db(select_sql)
    # print(contents)
    # 对得到的数据进行处理，以json格式返回给前端
